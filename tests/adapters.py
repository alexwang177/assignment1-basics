...
def run_embedding(
    ...
    table = Embedding(num_embeddings=vocab_size, embedding_dim=d_model, device=weights.device, dtype=weights.dtype)
    table.load_state_dict({"table": weights})
...
def run_scaled_dot_product_attention(
    ...
    """
    return scaled_dpa(Q=Q, K=K, V=V, mask=mask)
...
def run_multihead_self_attention_with_rope(
    ...
    return causal_mha(in_features, token_positions=token_positions)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return train_bpe(input_path=input_path, vocab_size=vocab_size, special_tokens=special_tokens, use_multiprocessing=True)
